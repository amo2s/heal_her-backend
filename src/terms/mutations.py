...
class LegalMutations:
    ...
    async def execute_document_signature(
        ...
        """
        Step 2: Verifies the OTP, releases the locked payload, and signs using a
        server-resolved IP — ip_address is no longer a client-suppliable argument.
        """
        clean_email = email.lower().strip()

        # Extract the AsyncSession + proxy-verified IP injected via the Strawberry context.
        db: AsyncSession = info.context["db"]
        verified_ip: str = info.context.get("verified_ip", "unknown")
