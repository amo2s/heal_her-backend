"""
src/teens/dashboard/resolvers.py

The Smart GraphQL Resolvers for the Teens Dashboard.
...
# [UPDATED IMPORTS]: Pulling in the newly upgraded schema structures
from .schemas import TeenIdentityResponse, TeenProfileType, TeenDashboardContext
...
def get_temporal_vibe(hour: int) -> dict:
    ...
    based on the server time (or user timezone).
    STRICT COMPLIANCE: Zero emojis used per system instructions.
    """
    if 5 <= hour < 12:
        return {
            "greeting": "Good Morning",
            "message": "Ready to start your day with a clear mind?"
        }
    elif 12 <= hour < 17:
        return {
            "greeting": "Good Afternoon",
            "message": "Taking a midday break? We are here for you."
        }
    elif 17 <= hour < 22:
        return {
            "greeting": "Good Evening",
            "message": "Winding down? Let us reflect on the day."
        }
    else:
        # Late night logic
        return {
            "greeting": "Hello",
            "message": "Still up? Remember to prioritize your rest."
        }
...
# =====================================================================
@strawberry.type
class TeenDashboardQueries:
    """
    The Command Center Read-Only Data Layer for Teens.
    """

    @strawberry.field
    async def get_me(self, info: Info) -> TeenIdentityResponse:
        """
        ...
        profile_data = TeenProfileType(
            first_name=first_name,
            full_name=raw_full_name,
            role=user_context.get("role", "teen").upper(),
            email=current_user.email
        )
        
        # Mapping Presentation Data
        context_data = TeenDashboardContext(
